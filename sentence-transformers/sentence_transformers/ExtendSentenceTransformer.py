...
from .models import Transformer, Pooling, Dense, ExtendTransformer
...
class ExtendSentenceTransformer(nn.Sequential):
    """
    ...
    def encode(self, sentences: Union[str, List[str]],
               ...
               graph_index=None, graph_type=None, pos_ids=None) -> Union[List[Tensor], ndarray, Tensor]:
        """
        ...
        for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
            ...
            if graph_index_sorted:
                features['edge_index'] = graph_index_sorted[start_index:start_index + batch_size]
                features['edge_type'] = graph_type_sorted[start_index:start_index + batch_size]
                features['pos_ids'] = pos_ids_sorted[start_index:start_index + batch_size]
            else:
        ...
        for example in batch:
            if example.edge_index:
                for idx, (text, edge_index, edge_type, pos_id) in enumerate(
                        zip(example.texts, example.edge_index, example.edge_type, example.pos_ids)):
        ...
        for loss_model in loss_models:
            ...
            optimizer_grouped_parameters = [
                {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay)],
                 # param_optimizer -> zip(params_name, params)
    ...
    def _load_auto_model(self, model_name_or_path, adapter_size, gnn, gnn_layer):
        """
        ...
        logger.warning(
            "No sentence-transformers model found with name {}. Creating a new one with MEAN pooling.".format(
                model_name_or_path))
        transformer_model = ExtendTransformer(model_name_or_path, adapter_size=adapter_size, gnn=gnn,
                                              gnn_layer=gnn_layer)  # modification
    ...
    @staticmethod
    def load(input_path):
        return ExtendSentenceTransformer(input_path)
