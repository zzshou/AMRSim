...
class ContrastiveTensionLoss(nn.Module):
    ...
    def __init__(self, model: ExtendSentenceTransformer):
        ...
        self.model1 = model  # This will be the final model used during the inference time.
...
class ContrastiveTensionExampleDataLoader:
        ...
        while example_idx + 1 < len(self.examples):
            if len(batch) % self.pos_neg_ratio > 0:  # Negative (different) pair
                ...
                if len(batch) >= self.batch_size:
                    ...
                    batch = []
                    example_idx += 1
                # elif self.aniso_in_batch:  # add negative example
                #     texts = s1.texts  # [original_linearized_amr_graph, reorder_linearized_amr_graph]
                #     edge_index, _ = s1.edge_index
                #     edge_type, _ = s1.edge_type
                #     batch.append(InputExample(texts=texts, label=0, edge_index=[edge_index, edge_index],
    ...
    def __len__(self):
        # if self.aniso_in_batch:
        #     tmp = math.floor(len(self.examples) / (2 * self.batch_size))
        #     return tmp + math.floor(tmp / self.batch_size)
